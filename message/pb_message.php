...
abstract class PBMessage {
        ...
        foreach (self::$fieldNames[$this->getClassNameWithoutNamespace($this) ] as $index => $fieldName) {
            ...
            if (is_array($this->values[$index]) && count($this->values[$index]) > 0) {
                ...
                foreach ($this->values[$index] as $repeated) {
                    ...
                    if ($isFirstInArray) $isFirstInArray = false;
                ...
                // wired and type
                if ($this->values[$index] instanceof PBScalar) $newstring = $this->values[$index]->toJson($fieldName);
                else $newstring = '"' . $fieldName . '":' . $this->values[$index]->toJson();
                $stringinner.= $newstring;
            } else {
                $validValue = false;
            }
    ...
    public function toXML($depth = 0, $rootOrInArray = true) {
        $outerTabs = "";
        for ($i = 0;$i < $depth;$i++) $outerTabs.= "\t";
        $innerTabs = "$outerTabs\t";
        $xml = "";
        if ($rootOrInArray) $xml.= "$outerTabs<" . $this->getClassNameWithoutNamespace($this, true) . ">\n";
    ...
    protected function _set_arr_values($index, $values) {
        ...
        foreach ($values as $value) $this->values[$index][] = $value;
    }
    /**
    ...
    protected function _get_value($index) {
        ...
        else if (is_array($this->values[$index])) return $this->values[$index];
        else return $this->values[$index]->value;
    }
    /**
     * Get array value
     * @param id of the field
    ...
    public function Send($url, &$class = null) {
        ...
        $result = curl_exec($ch);
        if ($class != null) $class->parseFromString($this->_d_string);
        return $this->_d_string;
    }
    protected function getClassNameWithoutNamespace($object, $toLower = false) {
        $classname = get_class($object);
        if ($toLower) $classname = strtolower($classname);
        $nameWithoutNamespace = strrchr($classname, "\\");
        if (!empty($nameWithoutNamespace)) return substr($nameWithoutNamespace, 1);
        ...
        if (isset($this->values)) {
            foreach ($this->values as $name => $value) {
                if (is_array($value)) {
                    ...
                    if (isset($name2)) unset($value->$name2);
                } else {
