...
class PBParser {
    ...
    var $c_types = array();
    // different typess
    var $scalar_types = array('double' => 'PB64Bit', 'float' => 'PB32Bit', 'int32' => 'PBInt', 'int64' => 'PBInt', 'uint32' => 'PBInt', 'uint64' => 'PBInt', 'sint32' => 'PBSignedInt', 'sint64' => 'PBSignedInt', 'fixed32' => 'PB32Bit', 'fixed64' => 'PB64Bit', 'sfixed32' => 'PB32Bit', 'sfixed64' => 'PB64Bit', 'bool' => 'PBBool', 'string' => 'PBString', 'bytes' => 'PBBytes', 'enum' => 'PBEnum');
    ...
    public function parse($protofile, $outputfile = null) {
        ...
        $name = join($name, '.');
        if (empty($outputfile)) $outputfile = 'pb_proto_' . $name . '.php';
    ...
    private function _create_class_file($filename) {
        $string = '';
        if (!empty($this->namespace)) $string.= "namespace " . str_replace("\\\\", "\\", $this->namespace) . ";\n";
        $requires_string = "";
        ...
        foreach ($this->m_types as $classfile) {
            ...
            if ($classfile['type'] == 'message') {
                if ($this->namespace) $string.= 'class ' . $classname . " extends \PBMessage\n{\n";
                else $string.= 'class ' . $classname . " extends PBMessage\n{\n";
                ...
                if ($this->namespace) $string.= 'class ' . $classname . " extends \PBEnum\n{\n";
                else $string.= 'class ' . $classname . " extends PBEnum\n{\n";
    ...
    private function _get_type($field) {
        if (isset($this->scalar_types[$field['value']['type']])) return $this->scalar_types[$field['value']['type']];
        else if (isset($this->c_types[$field['value']['namespace']])) return $this->c_types[$field['value']['namespace']];
        else if (isset($this->c_types[$field['value']['type']])) return $this->c_types[$field['value']['type']];
    ...
    private function _create_class_body($classfile, &$string, $classname) {
        foreach ($classfile as $field) {
            ...
            if (isset($field['value']['repeated']) && (isset($this->scalar_types[$field['value']['type']]) || $type == 'PBEnum')) {
    ...
    private function _create_class_definition($classfile, &$string) {
        ...
        foreach ($classfile as $field) {
            ...
            $ctorStr.= "\n\t\t\t" . $field['1'] . ' => "' . $field['0'] . '"';
            if ($i < count($classfile) - 1) $ctorStr.= ",";
            else $ctorStr.= ");\n";
            $i++;
        }
        $ctorStr.= "   }\n";
    ...
    private function _create_class_constructor($classfile, &$string, $classname) {
        if ($this->namespace) $string.= '  var $wired_type = \PBMessage::WIRED_LENGTH_DELIMITED;' . "\n";
        else $string.= '  var $wired_type = PBMessage::WIRED_LENGTH_DELIMITED;' . "\n";
        ...
        foreach ($classfile as $field) {
            ...
            if (isset($this->scalar_types[strtolower($classtype) ])) {
                ...
                if (!empty($this->namespace)) $classtype = "\\\\$classtype";
            } else if ((strpos($classtype, '_') === false)) {
                $classtype = str_replace('.', '_', $field['value']['namespace']);
                if (!empty($this->namespace)) $classtype = "\\\\$this->namespace\\\\$classtype";
            }
            ...
            if (!isset($field['value']['repeated']) && isset($field['value']['optional']) && isset($field['value']['default'])) {
                if (!empty($this->namespace)) $classtype = str_replace("\\\\", "\\", $classtype); // when createing object - it should not be escaped
                $string.= '    $this->values["' . $field['value']['value'] . '"] = new ' . $classtype . "();\n";
                if (isset($this->scalar_types[strtolower($_classtype) ])) $string.= '    $this->values["' . $field['value']['value'] . '"]->value = ' . $field['value']['default'] . '' . ";\n";
                // it must be an enum field perhaps type check
                else $string.= '    $this->values["' . $field['value']['value'] . '"]->value = ' . $classtype . '::' . $field['value']['default'] . '' . ";\n";
            }
    ...
    private function _parse_message_type(&$string, $m_name, $path = '') {
        ...
        while (strlen($string) > 0) {
            ...
            if (strtolower($next) == 'message') {
                ...
                $this->m_types[$newName] = array('name' => $newName, 'type' => 'enum', 'value' => $this->_parse_enum($content));
                ...
                if ($this->isPHP5_3) $this->namespace = str_replace(".", "\\\\", trim($name, ";"));
                //ignoring
                // removing it from string
                $offset = strlen($name);
                ...
                if (!$match) throw new Exception('Malformed include / look at your import statement:' . $string);
                $fn = $matches[1][0];
                if (!file_exists($fn)) {
                ...
                // now a normal field
                $match = preg_match('/(.*);\s?/', $string, $matches, PREG_OFFSET_CAPTURE);
                if (!$match) throw new Exception('Proto file missformed');
    ...
    private function _parse_field($content, $array, $path) {
        ...
        if ($match) {
            ...
            $content = trim(substr($content, 0, $matches[0][1]));
        } else throw new Exception('Protofile no value at ' . $content);
        // parse all modifier
        ...
        while (strlen($content) > 0) {
            ...
            $name = $matches[0][0];
            if (strtolower($name) == 'optional') $myarray['optional'] = true;
            else if (strtolower($name) == 'required') $myarray['required'] = true;
            else if (strtolower($name) == 'repeated') $myarray['repeated'] = true;
    ...
    private function _check_type($type, $array, $path) {
        if (isset($this->scalar_types[strtolower($type) ])) return array(strtolower($type), '');
        ...
        while (true) {
            ...
            if (isset($this->m_types[$nameprefix])) return array($type, $nameprefix);
            if (empty($tempPath)) break;
            $nameprefix = $tempPath;
        ...
        while (true) {
            $nameprefix = trim("$tempPath.$type", '.');
            if (isset($this->m_types_imported[$nameprefix])) return array($type, $nameprefix);
            if (empty($tempPath)) break;
            $nameprefix = $tempPath;
            $apath = explode("\.", $tempPath);
            array_pop($apath);
    ...
    private function _parse_enum($content) {
        ...
        if (!$match) throw new Execption('Semantic error in Enum!');
    ...
    private function _next($string, $reg = false) {
        $match = preg_match('/([^\s^\{}]*)/', $string, $matches, PREG_OFFSET_CAPTURE);
        if (!$match) return -1;
        if (!$reg) return (trim($matches[0][0]));
        else return $matches;
    }
    /**
    ...
    private function _get_begin_end($string, $char, $charend) {
        ...
        if ($offset_begin === false) return array('begin' => - 1, 'end' => - 1);
        $_offset_number = 1;
        ...
        if ($_offset == - 1) throw new Exception('Protofile failure: ' . $char . ' not nested');
